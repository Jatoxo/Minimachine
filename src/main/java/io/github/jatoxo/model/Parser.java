...
class Parser {
	...
	Parser(Scanner scanner, Speicher memory, FehlerVerwaltung exceptionHandler, boolean extended) {
		...
		this.curToken = this.scanner.nextToken();
	}
	...
	void parse() {
		...
		for(Entry<Integer, String> fixierung : this.fixierungen.entrySet()) {
			if(this.marken.containsKey(fixierung.getValue())) {
				this.speicher.WortSetzen(fixierung.getKey(), this.marken.get(fixierung.getValue()));
			} else {
				this.fehler.FehlerEintragen(R.string("parse_error_label_not_defined") + ": " + fixierung.getValue(), this.scanner.PositionGeben());
			}
		}
